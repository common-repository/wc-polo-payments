	...
	<tbody>
		<tr valign="top">
			<th scope="row" class="titledesc">
				...
				<p class="wc-polo-payments-info">
					<?php esc_html_e( 'Icone que aparece na tela de opções de pagamento em finalizar compra', 'wc-polo-payments' ); ?>
				</p>
			</th>
			<td class="forminp">
				<fieldset>
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				...
				<p class="wc-polo-payments-info">
					<?php esc_html_e( 'Quando é selecionado o PIX como forma de pagamento antes de finalizar a compra.', 'wc-polo-payments' ); ?>
				</p>
			</th>
			<td class="forminp">
				<fieldset>
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				<label for="<?php echo esc_html( $this->get_field_name( 'show_pix_image' ) ); ?>">Mostrar imagem PIX
					nas opções de pagamento
				</label>
				<p class="wc-polo-payments-info">
					<?php esc_html_e( 'A imagem do pix ficará acima do texto.', 'wc-polo-payments' ); ?>
				</p>
			</th>
			<td class="forminp">
				<fieldset>
					<label for="<?php echo esc_html( $this->get_field_name( 'show_pix_image' ) ); ?>">
						<input class="" type="checkbox"
							name="<?php echo esc_html( $this->get_field_name( 'show_pix_image' ) ); ?>"
							id="<?php echo esc_html( $this->get_field_name( 'show_pix_image' ) ); ?>" <?php echo esc_html( $this->show_pix_image == 'yes' ? 'checked' : '' ); ?>>Exibir a logo PIX oficial
						ao selecionar o pagamento.</label>
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				<label for="<?php echo esc_html( $this->get_field_name( 'order_recived_message' ) ); ?>">
					<?php echo esc_html( __( 'Mensagem na tela do QR Code', 'wc-polo-payments' ) ); ?>
				</label>
				<p class="wc-polo-payments-info">
					...
					echo wp_kses( sprintf( __( 'Essa mensagem aparece na tela do QR Code, depois que o cliente finaliza o pedido. %1$s para definir o local do código QR. %2$s para definir o local do botão para copiar o código. %3$s para definir o local do código pix em texto corrido %4$s para inserir a data que o código expira', 'wc-polo-payments' ), '<br><br><code>[qr_code]</code>', '<br><code>[copy_button]</code>', '<br><code>[text_code]</code>', '<br><code>[expiration_date]</code>' ), [ "code" => "", "br" => "" ] );
					?>
				</p>
			</th>
			<td class="forminp">
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				<label for="thank_you_message">
					<?php esc_html_e( 'Mensagem de agradecimento pelo pagamento', 'wc-polo-payments' ); ?>
				</label>
				<p class="wc-polo-payments-info">
					<?php esc_html_e( 'Essa mensagem aparece quando o pagamento PIX é confirmado.', 'wc-polo-payments' ); ?>
				</p>
			</th>
			<td class="forminp">
				<fieldset>
